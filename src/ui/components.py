        ...
        if selected_cat == extra_opt:
            ...
            if st.button("Add category"):
                if not new_category_name.strip():
                    ...
                    if added:
                        # Persisted: refresh UI so the new category shows in the dropdown.
                        st.success(f"Category '{new_category_name.strip()}' added. The form will refresh.")
                        _trigger_rerun()
                    else:
        ...
        if submit_button:
            ...
            if selected_cat == extra_opt:
                if new_category_name.strip():
                    ...
                    st.error("Please either pick an existing category or type a new one and press 'Add category'.")
                    return
